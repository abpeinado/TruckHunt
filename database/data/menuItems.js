/*
'Burgers: melts: hot dogs: burritos:sandwiches: fries: onion rings: drinks'
'Cold Truck: Hot & Cold Sandwiches: Bagels: Burritos: Soups: Hot Dogs: Tacos: Pork Buns: BBQ Meat: Fruit: Various Beverages: Pasties: Pre-Packaged Snacks: Candy: Salads: Muffins: Scones: Brownies: Croissants: Energy Bars: Noodle Bowls'
'Cold Truck: Breakfast: Sandwiches: Salads: Pre-Packaged Snacks: Beverages'
'Cold Truck: Hamburger: cheeseburgers: hot dogs: hot sandwiches: cold sandwiches: egg muffins: cup of noodles: corn dogs: canned soup: coffee: hot cocoa: hot tea: gatorade: juice: milk: soda: water: fruits: fruit salad: rice pudding: yogurt: candy bars: chips: cookies: donuts: granola bars: muffins'
'Cold Truck: Pre-packaged sandwiches: snacks: fruit: various beverages'
'Cold Truck: Corn Dogs: Noodle Soups: Candy: Pre-packaged Snacks: Sandwiches: Chips: Coffee: Tea: Various Beverages'
'Hot dogs: condiments: soft pretzels: soft drinks: coffee: cold beverages: pastries: bakery goods: cookies: ice cream: candy: soups: churros: chestnuts: nuts: fresh fruit: fruit juices: desserts: potato chips and popcorn.'
'Hot Dogs: Hamburgers: Nachos: Steaks: Pastas: Asian Dishes: Tri-Tip Sandwiches: Sodas & Water'
*/

module.exports.menuItems = [
/* -----------------------------Burgers 1---------------------------------*/
  {
    name: 'Corn Dog',
    item_description: 'Hot dog fried in corn meal',
    price: 399,
    course: 'Appetizers',
    food_category: 'Burgers: melts: hot dogs: burritos:sandwiches: fries: onion rings: drinks'
  },
  {
    name: 'Hot Dog Nuggets',
    item_description: 'Tiny hot dogs fried in corn meal',
    price: 499,
    course: 'Appetizers',
    food_category: 'Burgers: melts: hot dogs: burritos:sandwiches: fries: onion rings: drinks'
  },
  {
    name: 'Onion Rings',
    item_description: 'Beer battered and served with chipotle mayo',
    price: 299,
    course: 'Appetizers',
    food_category: 'Burgers: melts: hot dogs: burritos:sandwiches: fries: onion rings: drinks'
  },
  {
    name: 'Crinkle-Cut French Fries',
    item_description: 'Crinkle-Cut french fries',
    price: 299,
    course: 'Crinkle-Cut Fries',
    food_category: 'Burgers: melts: hot dogs: burritos:sandwiches: fries: onion rings: drinks'
  },
  {
    name: 'Crinkle-Cut Cheese Fries',
    item_description: 'Crinkle-Cut cheese fries',
    price: 299,
    course: 'Crinkle-Cut Fries',
    food_category: 'Burgers: melts: hot dogs: burritos:sandwiches: fries: onion rings: drinks'
  },
  {
    name: 'Bacon Cheese Fries',
    item_description: 'Fries with bacon and cheese',
    price: 299,
    course: 'Crinkle-Cut Fries',
    food_category: 'Burgers: melts: hot dogs: burritos:sandwiches: fries: onion rings: drinks'
  },
  {
    name: 'Bacon Ranch Fries',
    item_description: 'Fries with bacon and ranch',
    price: 299,
    course: 'Crinkle-Cut Fries',
    food_category: 'Burgers: melts: hot dogs: burritos:sandwiches: fries: onion rings: drinks'
  },
  {
    name: 'World Famous Beef Hot Dog',
    item_description: 'The Nathan\'s classic',
    price: 499,
    course: 'Entres',
    food_category: 'Burgers: melts: hot dogs: burritos:sandwiches: fries: onion rings: drinks'
  },
  {
    name: 'Chile Dog',
    item_description: 'Hot dog with chile',
    price: 599,
    course: 'Entres',
    food_category: 'Burgers: melts: hot dogs: burritos:sandwiches: fries: onion rings: drinks'
  },
  {
    name: 'Chile Cheese Dog',
    item_description: 'Hot dog with chile and cheese',
    price: 599,
    course: 'Entres',
    food_category: 'Burgers: melts: hot dogs: burritos:sandwiches: fries: onion rings: drinks'
  },
  {
    name: 'Super Cheeseburger',
    item_description: 'Cheeseburger with two patties',
    price: 699,
    course: 'Entres',
    food_category: 'Burgers: melts: hot dogs: burritos:sandwiches: fries: onion rings: drinks'
  },
  {
    name: 'Bacon Cheeseburger',
    item_description: 'Delicious bacon cheeseburger!',
    price: 699,
    course: 'Entres',
    food_category: 'Burgers: melts: hot dogs: burritos:sandwiches: fries: onion rings: drinks'
  },
  {
    name: 'The Original',
    item_description: 'Philly Cheesesteak',
    price: 799,
    course: 'Entres',
    food_category: 'Burgers: melts: hot dogs: burritos:sandwiches: fries: onion rings: drinks'
  },
/* --------------------------------Cold Truck 1------------------------------*/
  {
    name: 'All American Club',
    item_description: 'Roast turkey, honey ham, bacon, Monterey Jack cheese, lettuce, tomato and dijonaisse',
    price: 775,
    course: 'Wraps',
    food_category: 'Cold Truck: Corn Dogs: Noodle Soups: Candy: Pre-packaged Snacks: Sandwiches: Chips: Coffee: Tea: Various Beverages'
  },
  {
    name: 'Chicken Caesar',
    item_description: 'Grilled chicken, romaine lettuce, parmesan cheese and creamy Caesar dressing',
    price: 775,
    course: 'Wraps',
    food_category: 'Cold Truck: Corn Dogs: Noodle Soups: Candy: Pre-packaged Snacks: Sandwiches: Chips: Coffee: Tea: Various Beverages'
  },
  {
    name: 'Dijon Shrimp Salad',
    item_description: 'Large shrimp, mayo, mixed seasonings, lettuce and tomato',
    price: 775,
    course: 'Wraps',
    food_category: 'Cold Truck: Corn Dogs: Noodle Soups: Candy: Pre-packaged Snacks: Sandwiches: Chips: Coffee: Tea: Various Beverages'
  },
  {
    name: 'Bleu Cheese & Beef',
    item_description: 'Seasoned beef, sliced onions, lettuce tomato and bleu cheese dressing',
    price: 775,
    course: 'Wraps',
    food_category: 'Cold Truck: Corn Dogs: Noodle Soups: Candy: Pre-packaged Snacks: Sandwiches: Chips: Coffee: Tea: Various Beverages'
  },
  {
    name: 'Pit Sandwich',
    item_description: 'Pit style smoked turkey, honey ham and rop round roast beef piled high with cheese, lettuce and tomato on freshly baked Kaiser rolls.  Served with spicy mustard, mayo, BBQ sauce and horseradish on the side',
    price: 800,
    course: 'Sandwiches',
    food_category: 'Cold Truck: Corn Dogs: Noodle Soups: Candy: Pre-packaged Snacks: Sandwiches: Chips: Coffee: Tea: Various Beverages'
  },
  {
    name: 'Italian',
    item_description: 'Genoa salami, pepperoni, honey ham, parmesan cheese, lettuce, tomato, onion, banana peppers and Italian dressing',
    price: 800,
    course: 'Sandwiches',
    food_category: 'Cold Truck: Corn Dogs: Noodle Soups: Candy: Pre-packaged Snacks: Sandwiches: Chips: Coffee: Tea: Various Beverages'
  },
  {
    name: 'Black Russian',
    item_description: 'Seasoned beef, roasted turkey, Swiss cheese, lettuce tomato, onion and Russian dressing',
    price: 800,
    course: 'Sandwiches',
    food_category: 'Cold Truck: Corn Dogs: Noodle Soups: Candy: Pre-packaged Snacks: Sandwiches: Chips: Coffee: Tea: Various Beverages'
  },
  {
    name: 'Country Style Chicken Salad',
    item_description: 'Made with roasted chicken, lettuce and tomato',
    price: 800,
    course: 'Sandwiches',
    food_category: 'Cold Truck: Corn Dogs: Noodle Soups: Candy: Pre-packaged Snacks: Sandwiches: Chips: Coffee: Tea: Various Beverages'
  },
  {
    name: 'Fresh baked cookies',
    item_description: 'Chocolate chip, sugar and oatmeal raisin',
    price: 175,
    course: 'Desserts',
    food_category: 'Cold Truck: Corn Dogs: Noodle Soups: Candy: Pre-packaged Snacks: Sandwiches: Chips: Coffee: Tea: Various Beverages'
  },
  {
    name: 'Dessert tray',
    item_description: 'Fresh baked cookies, cream puffs & brownies',
    price: 225,
    course: 'Desserts',
    food_category: 'Cold Truck: Corn Dogs: Noodle Soups: Candy: Pre-packaged Snacks: Sandwiches: Chips: Coffee: Tea: Various Beverages'
  },
  {
    name: 'Fresh Fruit Salad ',
    item_description: 'Seasonal selection of fruit',
    price: 325,
    course: 'Desserts',
    food_category: 'Cold Truck: Corn Dogs: Noodle Soups: Candy: Pre-packaged Snacks: Sandwiches: Chips: Coffee: Tea: Various Beverages'
  },
/* --------------------------------Cold Truck 2------------------------------*/
  {
    name: 'All American Club',
    item_description: 'Roast turkey, honey ham, bacon, Monterey Jack cheese, lettuce, tomato and dijonaisse',
    price: 775,
    course: 'Wraps',
    food_category: 'Cold Truck: Hamburger: cheeseburgers: hot dogs: hot sandwiches: cold sandwiches: egg muffins: cup of noodles: corn dogs: canned soup: coffee: hot cocoa: hot tea: gatorade: juice: milk: soda: water: fruits: fruit salad: rice pudding: yogurt: candy bars: chips: cookies: donuts: granola bars: muffins'
  },
  {
    name: 'Chicken Caesar',
    item_description: 'Grilled chicken, romaine lettuce, parmesan cheese and creamy Caesar dressing',
    price: 775,
    course: 'Wraps',
    food_category: 'Cold Truck: Hamburger: cheeseburgers: hot dogs: hot sandwiches: cold sandwiches: egg muffins: cup of noodles: corn dogs: canned soup: coffee: hot cocoa: hot tea: gatorade: juice: milk: soda: water: fruits: fruit salad: rice pudding: yogurt: candy bars: chips: cookies: donuts: granola bars: muffins'
  },
  {
    name: 'Dijon Shrimp Salad',
    item_description: 'Large shrimp, mayo, mixed seasonings, lettuce and tomato',
    price: 775,
    course: 'Wraps',
    food_category: 'Cold Truck: Hamburger: cheeseburgers: hot dogs: hot sandwiches: cold sandwiches: egg muffins: cup of noodles: corn dogs: canned soup: coffee: hot cocoa: hot tea: gatorade: juice: milk: soda: water: fruits: fruit salad: rice pudding: yogurt: candy bars: chips: cookies: donuts: granola bars: muffins'
  },
  {
    name: 'Bleu Cheese & Beef',
    item_description: 'Seasoned beef, sliced onions, lettuce tomato and bleu cheese dressing',
    price: 775,
    course: 'Wraps',
    food_category: 'Cold Truck: Hamburger: cheeseburgers: hot dogs: hot sandwiches: cold sandwiches: egg muffins: cup of noodles: corn dogs: canned soup: coffee: hot cocoa: hot tea: gatorade: juice: milk: soda: water: fruits: fruit salad: rice pudding: yogurt: candy bars: chips: cookies: donuts: granola bars: muffins'
  },
  {
    name: 'Pit Sandwich',
    item_description: 'Pit style smoked turkey, honey ham and rop round roast beef piled high with cheese, lettuce and tomato on freshly baked Kaiser rolls.  Served with spicy mustard, mayo, BBQ sauce and horseradish on the side',
    price: 800,
    course: 'Sandwiches',
    food_category: 'Cold Truck: Hamburger: cheeseburgers: hot dogs: hot sandwiches: cold sandwiches: egg muffins: cup of noodles: corn dogs: canned soup: coffee: hot cocoa: hot tea: gatorade: juice: milk: soda: water: fruits: fruit salad: rice pudding: yogurt: candy bars: chips: cookies: donuts: granola bars: muffins'
  },
  {
    name: 'Italian',
    item_description: 'Genoa salami, pepperoni, honey ham, parmesan cheese, lettuce, tomato, onion, banana peppers and Italian dressing',
    price: 800,
    course: 'Sandwiches',
    food_category: 'Cold Truck: Hamburger: cheeseburgers: hot dogs: hot sandwiches: cold sandwiches: egg muffins: cup of noodles: corn dogs: canned soup: coffee: hot cocoa: hot tea: gatorade: juice: milk: soda: water: fruits: fruit salad: rice pudding: yogurt: candy bars: chips: cookies: donuts: granola bars: muffins'
  },
  {
    name: 'Black Russian',
    item_description: 'Seasoned beef, roasted turkey, Swiss cheese, lettuce tomato, onion and Russian dressing',
    price: 800,
    course: 'Sandwiches',
    food_category: 'Cold Truck: Hamburger: cheeseburgers: hot dogs: hot sandwiches: cold sandwiches: egg muffins: cup of noodles: corn dogs: canned soup: coffee: hot cocoa: hot tea: gatorade: juice: milk: soda: water: fruits: fruit salad: rice pudding: yogurt: candy bars: chips: cookies: donuts: granola bars: muffins'
  },
  {
    name: 'Country Style Chicken Salad',
    item_description: 'Made with roasted chicken, lettuce and tomato',
    price: 800,
    course: 'Sandwiches',
    food_category: 'Cold Truck: Hamburger: cheeseburgers: hot dogs: hot sandwiches: cold sandwiches: egg muffins: cup of noodles: corn dogs: canned soup: coffee: hot cocoa: hot tea: gatorade: juice: milk: soda: water: fruits: fruit salad: rice pudding: yogurt: candy bars: chips: cookies: donuts: granola bars: muffins'
  },
  {
    name: 'Fresh baked cookies',
    item_description: 'Chocolate chip, sugar and oatmeal raisin',
    price: 175,
    course: 'Desserts',
    food_category: 'Cold Truck: Hamburger: cheeseburgers: hot dogs: hot sandwiches: cold sandwiches: egg muffins: cup of noodles: corn dogs: canned soup: coffee: hot cocoa: hot tea: gatorade: juice: milk: soda: water: fruits: fruit salad: rice pudding: yogurt: candy bars: chips: cookies: donuts: granola bars: muffins'
  },
  {
    name: 'Dessert tray',
    item_description: 'Fresh baked cookies, cream puffs & brownies',
    price: 225,
    course: 'Desserts',
    food_category: 'Cold Truck: Hamburger: cheeseburgers: hot dogs: hot sandwiches: cold sandwiches: egg muffins: cup of noodles: corn dogs: canned soup: coffee: hot cocoa: hot tea: gatorade: juice: milk: soda: water: fruits: fruit salad: rice pudding: yogurt: candy bars: chips: cookies: donuts: granola bars: muffins'
  },
  {
    name: 'Fresh Fruit Salad ',
    item_description: 'Seasonal selection of fruit',
    price: 325,
    course: 'Desserts',
    food_category: 'Cold Truck: Hamburger: cheeseburgers: hot dogs: hot sandwiches: cold sandwiches: egg muffins: cup of noodles: corn dogs: canned soup: coffee: hot cocoa: hot tea: gatorade: juice: milk: soda: water: fruits: fruit salad: rice pudding: yogurt: candy bars: chips: cookies: donuts: granola bars: muffins'
  },
/* --------------------------------Cold Truck 3------------------------------*/
  {
    name: 'All American Club',
    item_description: 'Roast turkey, honey ham, bacon, Monterey Jack cheese, lettuce, tomato and dijonaisse',
    price: 775,
    course: 'Wraps',
    food_category: 'Cold Truck: Pre-packaged sandwiches: snacks: fruit: various beverages'
  },
  {
    name: 'Chicken Caesar',
    item_description: 'Grilled chicken, romaine lettuce, parmesan cheese and creamy Caesar dressing',
    price: 775,
    course: 'Wraps',
    food_category: 'Cold Truck: Pre-packaged sandwiches: snacks: fruit: various beverages'
  },
  {
    name: 'Dijon Shrimp Salad',
    item_description: 'Large shrimp, mayo, mixed seasonings, lettuce and tomato',
    price: 775,
    course: 'Wraps',
    food_category: 'Cold Truck: Pre-packaged sandwiches: snacks: fruit: various beverages'
  },
  {
    name: 'Bleu Cheese & Beef',
    item_description: 'Seasoned beef, sliced onions, lettuce tomato and bleu cheese dressing',
    price: 775,
    course: 'Wraps',
    food_category: 'Cold Truck: Pre-packaged sandwiches: snacks: fruit: various beverages'
  },
  {
    name: 'Pit Sandwich',
    item_description: 'Pit style smoked turkey, honey ham and rop round roast beef piled high with cheese, lettuce and tomato on freshly baked Kaiser rolls.  Served with spicy mustard, mayo, BBQ sauce and horseradish on the side',
    price: 800,
    course: 'Sandwiches',
    food_category: 'Cold Truck: Pre-packaged sandwiches: snacks: fruit: various beverages'
  },
  {
    name: 'Italian',
    item_description: 'Genoa salami, pepperoni, honey ham, parmesan cheese, lettuce, tomato, onion, banana peppers and Italian dressing',
    price: 800,
    course: 'Sandwiches',
    food_category: 'Cold Truck: Pre-packaged sandwiches: snacks: fruit: various beverages'
  },
  {
    name: 'Black Russian',
    item_description: 'Seasoned beef, roasted turkey, Swiss cheese, lettuce tomato, onion and Russian dressing',
    price: 800,
    course: 'Sandwiches',
    food_category: 'Cold Truck: Pre-packaged sandwiches: snacks: fruit: various beverages'
  },
  {
    name: 'Country Style Chicken Salad',
    item_description: 'Made with roasted chicken, lettuce and tomato',
    price: 800,
    course: 'Sandwiches',
    food_category: 'Cold Truck: Pre-packaged sandwiches: snacks: fruit: various beverages'
  },
  {
    name: 'Fresh baked cookies',
    item_description: 'Chocolate chip, sugar and oatmeal raisin',
    price: 175,
    course: 'Desserts',
    food_category: 'Cold Truck: Pre-packaged sandwiches: snacks: fruit: various beverages'
  },
  {
    name: 'Dessert tray',
    item_description: 'Fresh baked cookies, cream puffs & brownies',
    price: 225,
    course: 'Desserts',
    food_category: 'Cold Truck: Pre-packaged sandwiches: snacks: fruit: various beverages'
  },
  {
    name: 'Fresh Fruit Salad ',
    item_description: 'Seasonal selection of fruit',
    price: 325,
    course: 'Desserts',
    food_category: 'Cold Truck: Pre-packaged sandwiches: snacks: fruit: various beverages'
  },
/* --------------------------------Cold Truck (BH & MT)------------------------------*/
  {
    name: 'Jerked chicken brochettes',
    item_description: 'Jerked chicken brochettes with tangy scallion dipping sauce',
    price: 600,
    course: 'HORS D\'OEUVRES',
    food_category: 'Cold Truck: Breakfast: Sandwiches: Salads: Pre-Packaged Snacks: Beverages'
  },
  {
    name: 'Delicate Crostini',
    item_description: 'Crostini with sweet pea puree, ricotta and black pepper honey',
    price: 600,
    course: 'HORS D\'OEUVRES',
    food_category: 'Cold Truck: Breakfast: Sandwiches: Salads: Pre-Packaged Snacks: Beverages'
  },
  {
    name: 'Pea Fritters',
    item_description: 'Large shrimp, mayo, mixed seasonings, lettuce and tomato',
    price: 600,
    course: 'HORS D\'OEUVRES',
    food_category: 'Cold Truck: Breakfast: Sandwiches: Salads: Pre-Packaged Snacks: Beverages'
  },
  {
    name: 'Southern Ham',
    item_description: 'Honey and clove glazed southern ham served with hot mustard, tiny cheddar biscuits and watercress sauce',
    price: 1100,
    course: 'Entres',
    food_category: 'Cold Truck: Breakfast: Sandwiches: Salads: Pre-Packaged Snacks: Beverages'
  },
  {
    name: 'Corn Pudding Soufflés',
    item_description: 'Corn pudding soufflés',
    price: 400,
    course: 'Entres',
    food_category: 'Cold Truck: Breakfast: Sandwiches: Salads: Pre-Packaged Snacks: Beverages'
  },
  {
    name: 'Shrimp-Stuffed Baby Artichokes',
    item_description: 'Baby artichokes stuffed with spicy carolina shrimp, lemon and buttered breadcrumbs',
    price: 800,
    course: 'Entres',
    food_category: 'Cold Truck: Breakfast: Sandwiches: Salads: Pre-Packaged Snacks: Beverages'
  },
  {
    name: 'Lamb Chops',
    item_description: 'Grilled lamb chops with a kumquat and mint salsa',
    price: 1400,
    course: 'Entres',
    food_category: 'Cold Truck: Breakfast: Sandwiches: Salads: Pre-Packaged Snacks: Beverages'
  },
  {
    name: 'Gem Lettuce Salad',
    item_description: 'Gem lettuces with buttermilk and herb dressing',
    price: 700,
    course: 'Entres',
    food_category: 'Cold Truck: Breakfast: Sandwiches: Salads: Pre-Packaged Snacks: Beverages'
  },
  {
    name: 'Pots de Creme',
    item_description: 'Banana whiskey pots de creme with glazed pecans',
    price: 700,
    course: 'Desserts',
    food_category: 'Cold Truck: Breakfast: Sandwiches: Salads: Pre-Packaged Snacks: Beverages'
  },
  {
    name: 'Blackberry Lime Shortcakes',
    item_description: 'Individual blackberry lime shortcakes with sweet cream',
    price: 700,
    course: 'Desserts',
    food_category: 'Cold Truck: Breakfast: Sandwiches: Salads: Pre-Packaged Snacks: Beverages'
  },
/* --------------------------------Cold Truck (actual)------------------------------*/
  {
    name: 'All American Club',
    item_description: 'Roast turkey, honey ham, bacon, Monterey Jack cheese, lettuce, tomato and dijonaisse',
    price: 775,
    course: 'Wraps',
    food_category: 'Cold Truck: Hot & Cold Sandwiches: Bagels: Burritos: Soups: Hot Dogs: Tacos: Pork Buns: BBQ Meat: Fruit: Various Beverages: Pasties: Pre-Packaged Snacks: Candy: Salads: Muffins: Scones: Brownies: Croissants: Energy Bars: Noodle Bowls'
  },
  {
    name: 'Chicken Caesar',
    item_description: 'Grilled chicken, romaine lettuce, parmesan cheese and creamy Caesar dressing',
    price: 775,
    course: 'Wraps',
    food_category: 'Cold Truck: Hot & Cold Sandwiches: Bagels: Burritos: Soups: Hot Dogs: Tacos: Pork Buns: BBQ Meat: Fruit: Various Beverages: Pasties: Pre-Packaged Snacks: Candy: Salads: Muffins: Scones: Brownies: Croissants: Energy Bars: Noodle Bowls'
  },
  {
    name: 'Dijon Shrimp Salad',
    item_description: 'Large shrimp, mayo, mixed seasonings, lettuce and tomato',
    price: 775,
    course: 'Wraps',
    food_category: 'Cold Truck: Hot & Cold Sandwiches: Bagels: Burritos: Soups: Hot Dogs: Tacos: Pork Buns: BBQ Meat: Fruit: Various Beverages: Pasties: Pre-Packaged Snacks: Candy: Salads: Muffins: Scones: Brownies: Croissants: Energy Bars: Noodle Bowls'
  },
  {
    name: 'Bleu Cheese & Beef',
    item_description: 'Seasoned beef, sliced onions, lettuce tomato and bleu cheese dressing',
    price: 775,
    course: 'Wraps',
    food_category: 'Cold Truck: Hot & Cold Sandwiches: Bagels: Burritos: Soups: Hot Dogs: Tacos: Pork Buns: BBQ Meat: Fruit: Various Beverages: Pasties: Pre-Packaged Snacks: Candy: Salads: Muffins: Scones: Brownies: Croissants: Energy Bars: Noodle Bowls'
  },
  {
    name: 'Pit Sandwich',
    item_description: 'Pit style smoked turkey, honey ham and rop round roast beef piled high with cheese, lettuce and tomato on freshly baked Kaiser rolls.  Served with spicy mustard, mayo, BBQ sauce and horseradish on the side',
    price: 800,
    course: 'Sandwiches',
    food_category: 'Cold Truck: Hot & Cold Sandwiches: Bagels: Burritos: Soups: Hot Dogs: Tacos: Pork Buns: BBQ Meat: Fruit: Various Beverages: Pasties: Pre-Packaged Snacks: Candy: Salads: Muffins: Scones: Brownies: Croissants: Energy Bars: Noodle Bowls'
  },
  {
    name: 'Italian',
    item_description: 'Genoa salami, pepperoni, honey ham, parmesan cheese, lettuce, tomato, onion, banana peppers and Italian dressing',
    price: 800,
    course: 'Sandwiches',
    food_category: 'Cold Truck: Hot & Cold Sandwiches: Bagels: Burritos: Soups: Hot Dogs: Tacos: Pork Buns: BBQ Meat: Fruit: Various Beverages: Pasties: Pre-Packaged Snacks: Candy: Salads: Muffins: Scones: Brownies: Croissants: Energy Bars: Noodle Bowls'
  },
  {
    name: 'Black Russian',
    item_description: 'Seasoned beef, roasted turkey, Swiss cheese, lettuce tomato, onion and Russian dressing',
    price: 800,
    course: 'Sandwiches',
    food_category: 'Cold Truck: Hot & Cold Sandwiches: Bagels: Burritos: Soups: Hot Dogs: Tacos: Pork Buns: BBQ Meat: Fruit: Various Beverages: Pasties: Pre-Packaged Snacks: Candy: Salads: Muffins: Scones: Brownies: Croissants: Energy Bars: Noodle Bowls'
  },
  {
    name: 'Country Style Chicken Salad',
    item_description: 'Made with roasted chicken, lettuce and tomato',
    price: 800,
    course: 'Sandwiches',
    food_category: 'Cold Truck: Hot & Cold Sandwiches: Bagels: Burritos: Soups: Hot Dogs: Tacos: Pork Buns: BBQ Meat: Fruit: Various Beverages: Pasties: Pre-Packaged Snacks: Candy: Salads: Muffins: Scones: Brownies: Croissants: Energy Bars: Noodle Bowls'
  },
  {
    name: 'Fresh baked cookies',
    item_description: 'Chocolate chip, sugar and oatmeal raisin',
    price: 175,
    course: 'Desserts',
    food_category: 'Cold Truck: Hot & Cold Sandwiches: Bagels: Burritos: Soups: Hot Dogs: Tacos: Pork Buns: BBQ Meat: Fruit: Various Beverages: Pasties: Pre-Packaged Snacks: Candy: Salads: Muffins: Scones: Brownies: Croissants: Energy Bars: Noodle Bowls'
  },
  {
    name: 'Dessert tray',
    item_description: 'Fresh baked cookies, cream puffs & brownies',
    price: 225,
    course: 'Desserts',
    food_category: 'Cold Truck: Hot & Cold Sandwiches: Bagels: Burritos: Soups: Hot Dogs: Tacos: Pork Buns: BBQ Meat: Fruit: Various Beverages: Pasties: Pre-Packaged Snacks: Candy: Salads: Muffins: Scones: Brownies: Croissants: Energy Bars: Noodle Bowls'
  },
  {
    name: 'Fresh Fruit Salad',
    item_description: 'Seasonal selection of fruit',
    price: 325,
    course: 'Desserts',
    food_category: 'Cold Truck: Hot & Cold Sandwiches: Bagels: Burritos: Soups: Hot Dogs: Tacos: Pork Buns: BBQ Meat: Fruit: Various Beverages: Pasties: Pre-Packaged Snacks: Candy: Salads: Muffins: Scones: Brownies: Croissants: Energy Bars: Noodle Bowls'
  },
/* -------------------------------Hot dogs 1-------------------------------*/
  {
    name: 'Classic Beef Hot Dog',
    item_description: 'hot dog with choice of toppings',
    price: 599,
    course: 'Hot Dogs',
    food_category: 'Hot Dogs: Hamburgers: Nachos: Steaks: Pastas: Asian Dishes: Tri-Tip Sandwiches: Sodas & Water'
  },
  {
    name: 'Chile Dog',
    item_description: 'Hot dog with chile',
    price: 599,
    course: 'Hot Dogs',
    food_category: 'Hot Dogs: Hamburgers: Nachos: Steaks: Pastas: Asian Dishes: Tri-Tip Sandwiches: Sodas & Water'
  },
  {
    name: 'Chile Cheese Dog',
    item_description: 'Hot dog with chile and cheese',
    price: 599,
    course: 'Entres',
    food_category: 'Hot Dogs: Hamburgers: Nachos: Steaks: Pastas: Asian Dishes: Tri-Tip Sandwiches: Sodas & Water'
  },
/* -------------------------------Hot dogs (actual)-------------------------------*/
  {
    name: 'Classic Beef Hot Dog',
    item_description: 'hot dog with choice of toppings',
    price: 599,
    course: 'Hot Dogs',
    food_category: 'Hot dogs: condiments: soft pretzels: soft drinks: coffee: cold beverages: pastries: bakery goods: cookies: ice cream: candy: soups: churros: chestnuts: nuts: fresh fruit: fruit juices: desserts: potato chips and popcorn.'
  },
  {
    name: 'Chile Dog',
    item_description: 'Hot dog with chile',
    price: 599,
    course: 'Hot Dogs',
    food_category: 'Hot dogs: condiments: soft pretzels: soft drinks: coffee: cold beverages: pastries: bakery goods: cookies: ice cream: candy: soups: churros: chestnuts: nuts: fresh fruit: fruit juices: desserts: potato chips and popcorn.'
  },
  {
    name: 'Chile Cheese Dog',
    item_description: 'Hot dog with chile and cheese',
    price: 599,
    course: 'Entres',
    food_category: 'Hot dogs: condiments: soft pretzels: soft drinks: coffee: cold beverages: pastries: bakery goods: cookies: ice cream: candy: soups: churros: chestnuts: nuts: fresh fruit: fruit juices: desserts: potato chips and popcorn.'
  }
/*--------------------------------------------------------------*/
];
